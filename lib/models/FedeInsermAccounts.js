...
function FedeInsermAccounts(client) {
  ...
  const selectEzTicketInfoForId = function* selectEzTicketInfoForId(...args) {
    ...
    const [query] = yield client.query({
      sql: `SELECT individual_account_fede.inserm_email, structures.code AS structure_code, teams.team_number, institute.name AS it_principal, regionals_delegations.code AS dr_code, section_cn.code AS section_cn_code
