...
function FedeInsermAccounts(client) {
  ...
  const selectEzTicketInfoForId = function* selectEzTicketInfoForId(...args) {
    ...
    const [query] = yield client.query({
      sql: `SELECT individual_account_fede.inserm_email, structures.code AS structure_code, teams.team_number, institute.code AS it_principal, regionals_delegations.code AS dr_code, section_cn.code AS section_cn_code
    ...
    return {
      username: `${user.inserm_email}_O_INSERM_OU_${query.structure_code ||
