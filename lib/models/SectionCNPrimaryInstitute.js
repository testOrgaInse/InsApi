...
function SectionCNPrimaryInstitute(client) {
  const sectionCNPrimaryInstituteClient = client.link(
    SectionCNPrimaryInstitute.queries
  );

  const assignPrimaryInstituteToSectionCN = function*(units, sectionCNId) {
    return yield sectionCNPrimaryInstituteClient.batchUpsert(
  ...
  const unassignPrimaryInstituteFromSectionCN = function*(
    instituteIds,
    sectionCNId
  ) {
    return yield sectionCNPrimaryInstituteClient.batchDelete(
