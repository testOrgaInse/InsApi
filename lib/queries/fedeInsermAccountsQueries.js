...
const selectBySimilarUid = uid => ({
  sql: `SELECT * FROM individual_account_fede WHERE uid LIKE $uid ORDER BY last_connection DESC, uid DESC`,
  parameters: {
    uid: `${uid}%`
  }
});
...
accountsFedeInsermQueries.selectPage
  .table(
    "individual_account_fede LEFT JOIN teams ON teams.id = individual_account_fede.team_number"
  )
  .returnFields(fields);
