...
const postgresCrud = (
  ...
  const defaultSearchableField = queriesFactory.queries.selectPage.searchableFields();
  const app = koa();
  let queries;
  ...
  app.use(function*(next) {
    ...
    queriesFactory.queries.selectPage.searchableFields(defaultSearchableField);
  ...
  app.use(
    koaRoute.get("/", function*() {
      ...
      // add join dynamically for filter who need them based on filtersJoin
      if (queriesFactory.queries.filtersJoin) {
        const jointFields = Object.keys(filters).filter(
          field => queriesFactory.queries.filtersJoin[field]
        );

        jointFields.forEach(field =>
          queriesFactory.queries.selectPage.searchableFields([
            ...queriesFactory.queries.selectPage.searchableFields(),
            field
          ])
        );

        const joins = jointFields
      ...
      const totalCount = this.body[0]
        ? this.body[0].totalcount
        : yield queries.countAll();
