import { insadmin_host, mailServer } from "config";

const getEditHref = id => `${insadmin_host}/#/janusAccounts/edit/${id}`;
const getListHref = uid => {
  const [, root] = uid.match(/^(.*?)\.[0-9]+$/) || [null, uid];
  return `${insadmin_host}/#/janusAccounts/list?search=%7B%22like_janus_account.uid%22:%22${root}%22%7D`;
};
