    ...
    try {
      ...
      yield histories.map(function*({
        ...
        try {
          ...
          const searchArticle = searchArticleFactory(community, ebscoToken);

          const retrieveArticle = retrieveArticleFactory(community, ebscoToken);
          ...
          const newRawRecords = getMissingResults(fullResult, last_results);
          ...
          yield historyQueries.updateOne(
            { id },
            {
              last_results: JSON.stringify(getResultsIdentifiers(fullResult)),
