...
describe("/ezticket", function() {
  ...
  before(function*() {
    ...
    const { id: instituteId } = yield fixtureLoader.createInstitute({
      code: "institute",
      name: "name"
    });
    const { id: unitId } = yield fixtureLoader.createUnit({
      code: "unit",
      name: "name"
    });
    ...
    unauthorizedUser = yield fixtureLoader.createInistAccount({
      username: "jane",
      password: "secret",
      communities: [shs.id]
    });
  ...
  it("should redirect to ezticket/login", function*() {
    ...
    assert.equal(
      response.body,
      'Redirecting to <a href="ezticket/login?gate=insb.test.com&amp;url=google.fr">ezticket/login?gate=insb.test.com&amp;url=google.fr</a>.'
    );
  });
  ...
  it("should return error 500 if gate does not exists", function*() {
    ...
    assert.equal(
      response.body,
      errorTemplate("en", "invalidGate", "fake.test.com")
    );
  });
  ...
  it("should redirect to generated url when correct inist cookie is present", function*() {
    request.setToken({
      ...
      domains: ["vie", "shs"],
      groups: ["insb", "inshs", "reaxys"]
    });

    const response = yield request.get(
      "/ezticket?gate=insb.test.com&url=http://google.fr"
    );
    assert.match(
      response.body,
      /Redirecting to.*?http:\/\/insb\.test\.com\/login\?user=johnny_O_UNKNOWN_I_institute_OU_unit.*?%24ginsb%2Binshs%2Breaxys/
    );
  });
  ...
  it("should redirect to generated url when correct janus cookie is present", function*() {
    ...
    assert.match(
      response.body,
      /Redirecting to.*?http:\/\/insb\.test\.com\/login\?user=johnny%40inist\.fr_O_CNRS_I_institute_OU_unit.*?%24ginsb%2Binshs%2Breaxys/
    );
  });
  ...
  it("should redirect to ezticket/login when logged user has no access to domain", function*() {
    ...
    const response = yield request.get(
      "/ezticket?gate=inc.test.com&url=google.fr",
      token
    );
    assert.equal(
      response.body,
      'Redirecting to <a href="ezticket/login?gate=inc.test.com&amp;url=google.fr">ezticket/login?gate=inc.test.com&amp;url=google.fr</a>.'
    );
  });
  ...
  describe("login", function() {
    ...
    it("should return 401 when posting /login a user with no access to the current gate", function*() {
      ...
      assert.match(
        response.body,
        /You cannot access this resource because you are searching in a discipline which is not within your authorized discipline field/
      );
    });
