...
describe("model Revue", function() {
  ...
  describe("selectRevueByDomains", function() {
    beforeEach(function*() {
      const [insb, inshs, inp] = yield ["INSB", "INSHS", "INP"].map(name =>
    ...
    it("should return revue for given domains", function*() {
      assert.deepEqual(yield revueQueries.selectRevueByDomains(["INSB"]), [
        {
          title: "Revue INSB",
          url: "http://insb.bib.cnrs.fr/login?url=http://www.INSB.com"
        }
      ]);
      assert.deepEqual(yield revueQueries.selectRevueByDomains(["INSHS"]), [
        {
          title: "Revue INSHS",
    ...
    it("duplicate should be removed and first matching community gate applied", function*() {
      assert.deepEqual(
        yield revueQueries.selectRevueByDomains(["INSB", "INSHS", "INP"]),
        [
          {
            title: "Revue INSB",
            url: "http://insb.bib.cnrs.fr/login?url=http://www.INSB.com"
      ...
      assert.deepEqual(
        yield revueQueries.selectRevueByDomains(["INP", "INSB", "INSHS"]),
        [
          {
            title: "Revue INSB",
