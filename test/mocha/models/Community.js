...
describe("model Community", function() {
  ...
  describe("selectByName", function() {
    it("should return each community with given names", function*() {
      const [insb, inshs, , inc] = yield ["insb", "inshs", "in2p3", "inc"].map(
        name => fixtureLoader.createCommunity({ name })
      );
  ...
  describe("selectOneByName", function() {
    it("should return community for given name", function*() {
      ...
      assert.deepEqual(yield communityQueries.selectOneByName("inshs"), inshs);
    });
  ...
  describe("selectByJanusAccountIdQuery", function() {
    it("should return community of user", function*() {
      const [insb, inshs, inc] = yield ["insb", "inshs", "inc"].map(name =>
        fixtureLoader.createCommunity({ name, gate: name })
      );
      ...
      assert.deepEqual(yield communityQueries.selectByJanusAccountId(jane.id), [
        {
          ...inc,
          totalcount: "2",
          index: 0,
  ...
  describe("selectByInistAccountIdQuery", function() {
    it("should return community of inistAccount", function*() {
      ...
      assert.deepEqual(yield communityQueries.selectByInistAccountId(jane.id), [
        {
          ...inc,
          totalcount: "2",
          index: 0,
  ...
  describe("selectByInstituteIdQuery", function() {
    it("should return community of institute", function*() {
      ...
      assert.deepEqual(yield communityQueries.selectByInstituteId(human.id), [
        {
          ...inc,
          totalcount: "2",
          index: 0,
