...
describe("parseXML", function() {
  ...
  describe("parseXMLObject", function() {
    ...
    it("should discard use term as value if no $ text", function() {
      assert.deepEqual(
        parseXMLObject({
          $name: "searchlink",
          $attrs: {
  ...
  describe("smartConcat", function() {
    it("should add value to array", function() {
      assert.deepEqual(smartConcat([{ item: 1 }], "new value"), [
        { item: 1 },
        "new value"
      ]);
    });
    ...
    it("should join value with array last item if both are string", function() {
      assert.deepEqual(smartConcat([{ item: 1 }, "join"], "us"), [
        { item: 1 },
        "join us"
      ]);
    });
  ...
  describe("parseXMLLine", function() {
    ...
    it("should return lastValue and indice extracted from string if there is no searchLink tag but one relatesTo tag", function*() {
      assert.deepEqual(
        yield parseXMLLine("<relatesTo>indice</relatesTo> after"),
        [
          {
            indice: "indice"
          },
    ...
    it("should use parseLabelValue if it start with <i>", function*() {
      assert.deepEqual(yield parseXMLLine("<i>label</i>value"), [
        "label value"
      ]);
    });
    ...
    it("should return given line if parseXMLLine cannot parse xml", function*() {
      assert.deepEqual(
        yield parseXMLLine("<relatesTo>indice</relatesTo> after </10 value"),
        "<relatesTo>indice</relatesTo> after </10 value"
      );
    });
  });
});
